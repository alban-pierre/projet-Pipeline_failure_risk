function algo = algo_options()
    ...
    algo.regr_hyp = 0.4; % Hyperparameter of a regression

    algo.error = 0; % AUC error
    %algo.error = 1; % Absolute error
    %algo.error = 2; % Square error

    algo.deep.sizes = [12,100,70,20,2];
    ...
    algo.deep.learn_rate = @(k) 10/sqrt(k); % A coefficient, which describe the learning rate
    algo.deep.epoch = 200; % The number of epochs
